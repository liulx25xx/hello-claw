...
> **提示**：Dashboard 编辑的就是 `~/.openclaw/openclaw.json` 文件。你也可以用命令行 `openclaw config set <key> <value>` 或直接编辑文件，效果完全一样（详见[第七章 配置管理](/cn/adopt/chapter7/#_2-配置管理)）。

### 远程访问 Dashboard

默认情况下 Dashboard 只能在本机访问。如果你的 Gateway 运行在远程服务器上，需要通过 SSH 隧道或 Tailscale 访问（详见[第八章 远程访问](/cn/adopt/chapter8/)）：
...
> **安全提醒**：Dashboard 拥有完整的管理权限。务必设置认证，尤其是 Gateway 不在 loopback 上运行时（详见[第九章 安全防护](/cn/adopt/chapter9/)）。
...
WebChat 最适合**本地开发和测试**。如果你需要移动端推送通知或群聊功能，建议搭配 Telegram 或 Discord（详见[第三章 Chat Provider 配置](/cn/adopt/chapter3/)）。
...
然后将第三方客户端的 API 地址指向 `http://127.0.0.1:18789/v1/chat/completions`，使用 Gateway 认证 token 作为 API Key（详见[第七章 HTTP API 端点](/cn/adopt/chapter7/#_8-http-api-端点)）。
...
> **提示**：远程模式的详细配置参见[第八章 远程访问](/cn/adopt/chapter8/)。
...
3. 确认模型配置正确、API Key 有效（详见[第四章 Models 配置](/cn/adopt/chapter4/)）

**Q: 远程服务器上能用 WebChat 吗？**

A: 可以。建立 SSH 隧道后，在本地浏览器访问 `http://localhost:18789` 即可。Tailscale 用户可以直接通过 Tailscale IP 访问（详见[第八章 远程访问](/cn/adopt/chapter8/)）。
...
**下一步**：
- 配置首次使用向导 → [第十一章 Onboarding 配置向导](/cn/adopt/chapter11/)
