...
| **买粮食** | 给员工发工资 | 配置模型 API Key（Token 就是龙虾的"口粮"） | [第 2 章](/cn/adopt/chapter2/) / [第 4 章](/cn/adopt/chapter4/) |
| **给联系方式** | 让客户能找到员工 | 接入聊天平台（QQ / 飞书 / Telegram） | [第 3 章](/cn/adopt/chapter3/) |
| **培训上岗** | 教员工怎么干活 | 配置 Agent、工具、定时任务 | [第 5 章](/cn/adopt/chapter5/) / [第 6 章](/cn/adopt/chapter6/) |
...
- **🔵 安装**（第 1～2 章）——第 1 章下载 AutoClaw 桌面客户端，5 分钟零门槛体验；第 2 章手动安装 OpenClaw，完全掌控配置和数据。
- **🟢 场景配置**（第 3～11 章）——按功能模块逐步配置：渠道接入（Ch3）、模型选择（Ch4）、Agent 管理（Ch5）、工具与定时任务（Ch6）、Gateway 运维（Ch7）、远程访问（Ch8）、安全防护（Ch9）、Web 界面（Ch10）、Onboarding 向导（Ch11）。装好后按需选读，不必按顺序。
